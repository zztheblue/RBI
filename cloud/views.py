...
def FullyConsequenceMana(request, proposalID):
    ...
    try:
        ...
        if isBottom:
            ...
            if request.method == 'POST':
                return redirect('verifullyConsequenceMana', proposalID)
            return render(request, 'ManagerUI/RiskSummaryMana/fullySellConsequenceMana.html', {'page':'fullyConse', 'data': data, 'proposalID':proposalID, 'ass':rwAss,'count':count,'noti':noti,'countnoti':countnoti,'info':request.session})
        else:
            ca = models.RwCaLevel1.objects.get(id= proposalID)
            inputCa = models.RwInputCaLevel1.objects.get(id= proposalID)
            data['production_cost'] = roundData.roundMoney(inputCa.production_cost)
        ...
        if isBottom:
            ...
            if 'Verifica' in request.POST:
                ...
                for some_var in some_var:
                    ...
                    vericontent.save()
                return HttpResponse("Bạn đã yêu cầu kiểm định thành công")
            return render(request, 'ManagerUI/verification_requirments/fullySellConsequenceVerification.html', {'page':'fullyConse', 'data': data, 'proposalID':proposalID, 'ass':rwAss,'count':count,'noti':noti,'countnoti':countnoti,'info':request.session})
        else:
            ca = models.RwCaLevel1.objects.get(id= proposalID)
            inputCa = models.RwInputCaLevel1.objects.get(id= proposalID)
            data['production_cost'] = roundData.roundMoney(inputCa.production_cost)
